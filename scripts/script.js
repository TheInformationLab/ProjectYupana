...
function countUsers() {
	tableauDB.fetchRecords(0,"users", function(users) {
		document.getElementById("item user").innerHTML = "<div class='countValue'><h2>"+users.length+"</h2></div><div class='countTitle'>users</div>"
	});
}
...
function getUsers_noAPI() {
	...
	$.ajax(settings).done(function (response) {
		...
		for (var i = 0, user; user = users[i]; i++) {
			tableauDB.createUser(user.username, user.displayName, "", user.siteRole, "", "", "", "", function() {
				currentUser++;
	...
	$.ajax(settings).done(function (response) {
		...
		if (curGroupCount == 0) {
			...
			for (var i = 0, group; group = groups[i]; i++) {
				tableauDB.createGroup(group.id, group.name, group.domainName, currentSiteId, function() {
		...
		if (views) {
			...
			for (var i = 0; i < curViewCount; i++){
				...
				$.ajax(settings).done(function (response) {
					...
					tableauDB.createView(v["id"], v["name"], v["index"], v.createdAt, v.editUrl, v.favorite, v.hitsTimeSeries, viewUsage.favoritesTotal, viewUsage.hitsLastOneMonthTotal, viewUsage.hitsLastThreeMonthsTotal, viewUsage.hitsLastTwelveMonthsTotal, viewUsage.hitsTotal, v.owner["id"], v.path, v.project["id"], v["id"], v.thumbnailUrl, v.updatedAt, v.workbook["id"], currentSiteId, function() {
	...
	$.ajax(settings).done(function (response) {
		...
		if (workbooks) {
			...
			for (var i = 0; i < curWorkbookCount; i++){
				...
				var w  = workbook;
				tableauDB.createWorkbook(w["id"], w["name"], w["size"], w.downloadUrl, w.defaultViewId, w.defaultViewUrl, w.description, w.displayTabs, w.hasAlert, w.hasExtracts, w.ownerId, w.projectId, w.repositoryUrl, w.sheetCount, w.thumbnailUrl, w.updatedAt, w.favoritesTotal, w.hitsLastOneMonthTotal, w.hitsLastThreeMonthsTotal, w.hitsLastTwelveMonthsTotal, w.hitsTotal, w.subscriptionsTotal, currentSiteId, function() {
					console.log("Workbook "+name+" saved!");
					currentWorkbook++;
					if (currentWorkbook == curWorkbookCount) {
						console.log("All workbookes saved!");
						document.getElementById("workbook").innerHTML = "<div class='countValue'><h2>"+workbookCount+"</h2></div><div class='countTitle'>workbooks</div>"
...
function initiliseStatsTiles() {
		...
		workbookCountDiv.setAttribute('id','workbook');
...
function drawChart(stats) {
	...
	var width = document.getElementById('workbook').offsetWidth;
	statsbarchart
...
function showEmailForm() {
	var formContainer = document.querySelector('#emailContainer');
